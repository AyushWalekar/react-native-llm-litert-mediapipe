...
class ExpoLlmMediapipeModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    // Create model from downloaded file with optional multimodal support
    AsyncFunction("createModelFromDownloaded") { modelName: String, maxTokens: Int?, topK: Int?, temperature: Double?, randomSeed: Int?, options: Map<String, Any>?, promise: Promise ->
      val modelFile = getModelFile(modelName)
      ...
      val enableVisionModality = (options?.get("enableVisionModality") as? Boolean) ?: false
      val enableAudioModality = (options?.get("enableAudioModality") as? Boolean) ?: false
      val maxNumImages = (options?.get("maxNumImages") as? Number)?.toInt() ?: 10
    ...
    // Add image to session from file path
    AsyncFunction("addImageToSession") { handle: Int, imagePath: String, promise: Promise ->
      try {
        val model = modelMap[handle]
        if (model == null) {
          promise.reject("INVALID_HANDLE", "No model found for handle $handle", null)
    ...
    // Add audio to session from file path
    AsyncFunction("addAudioToSession") { handle: Int, audioPath: String, promise: Promise ->
      try {
        val model = modelMap[handle]
        if (model == null) {
          promise.reject("INVALID_HANDLE", "No model found for handle $handle", null)
