...
## What is React Native MediaPipe LLM?

React Native MediaPipe LLM is a package that enables developers to run Large Language Models (LLMs) locally on Android and iOS devices using Google’s MediaPipe API. It provides both **React Hooks** for declarative usage and **Manual APIs** for advanced use cases.
...
To install the **React Native MediaPipe LLM** package, follow the steps below. Choose your preferred package manager.
...
```bash
pnpm add react-native-llm-litert-mediapipe
```
